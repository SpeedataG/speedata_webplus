...
<body>
    ...
    <div id="mainLayout" class="easyui-layout hidden" data-options="fit: true">
        <div id="northPanel" data-options="region: 'north', border: false" style="height: 65px; overflow: hidden;">
            <div id="topbar" class="top-bar" style="height:65px;">
                <div class="top-bar-left" style="height:65px;">
                   <img src="${ctx}/static/images/logo_xj.gif" style="text-align:center;vertical-align:middle;width:150px;padding-top:12px;padding-left:10px;"/> <span style="color:#fff;padding-top:10px;padding-left:6px;vertical-align:middle;font-size:20px;font-weight:bold;">管理服务平台</span>
                </div>
                <div class="top-bar-right" style="line-height:65px;">
                
                    <div id="timerSpan" style="padding:0;line-height:65px;height:65px;">  
                     <span class="icon-hamburg-user" style="padding-left: 25px;color:#fff;vertical-align:middle; background-position: left center;">
                       <shiro:principal property="name"/>，您好  <span id="btnExit" data-options="iconCls:'logout'">退出系统</span>
                    </span>
                    </div>
                    <div id="themeSpan">
            ...
            <div id="toolbar" class="panel-header panel-header-noborder top-toolbar" style="display:none;">
                <div id="infobar">
        ...
        <div data-options="region: 'west',iconCls: 'glyphicon-cloud', title: '工作控制台',  split: true, minWidth: 160, maxWidth: 400" style="width: 180px; padding: 1px;">
          ...
          <!--   <script id="menu" type="text/html">
			...
			{{each data as p_permission}}
				{{if (p_permission.pid==null)}}
   				 <div title="{{p_permission.name }}" style="padding: 5px;" data-options="border:false,iconCls:'glyphicon-plus'">
					<div>
					{{each data as c_permission}}	
						{{if (c_permission.pid==p_permission.id)}}
						<a id="btn" class="easyui-linkbutton" data-options="plain:true,iconCls:'glyphicon-list'" style="width:98%;margin-bottom:5px;" onclick="window.mainpage.mainTabs.addModule('{{c_permission.name}}','{{c_permission.url }}','{{c_permission.icon }}')">{{c_permission.name}}</a>
						{{/if}}	
					{{/each}}
					</div>
				</div>
